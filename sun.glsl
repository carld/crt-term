...
float pn( in vec3 p )
{
    ...
    uv = vec2(3.13) * rand(vec2(3.14).xy); //texture2D( tex, (uv+ 0.5)/256.0, -100.0 ).yx;
...
float SunSurface( in vec3 pos )
{
    ...
    r1.y = pos.y-0.1 - 0.7*h + 0.5*sin( 3.0*appTime+pos.x + 3.0*pos.z);
...
float map(vec3 p) {
  float y = .5, x= .5;
   p.z += 1.;
   R(p.yz, -25.5);// -1.0+ y *0.003);
   R(p.xz, x*0.008*pi+appTime*0.1);
   return SunSurface(p) +  fpn(p*50.+appTime*25.) * 0.45;
}
...
{  
   ...
   vec3 rd = normalize(vec3((gl_FragCoord.xy-0.5*targetSize.xy)/targetSize.y, 1.));
   vec3 ro = vec3(0., 0., -22.);
   ...
   #ifdef DITHERING
   vec2 pos = ( Texcoord.xy / targetSize.xy );
   vec2 seed = pos + fract(appTime);
   //t=(1.+0.2*rand(seed));
   #endif
  ...
  outColor = mix(texture(tex, Texcoord), vec4(tc, 1.0), 0.3);
}
